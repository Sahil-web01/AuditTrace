...
const sampleInvoices = [
  // 1. TechLogix Cloud (Cloud & Infrastructure)
  {
  ...
  // OUTLIER 1: Extreme Price Outlier ($14,950 vs ~$1,300 normal)
  {
    vendorName: "TechLogix Cloud Services",
...
const seedDatabase = async () => {
  try {
    const uri = process.env.MONGO_URI || "mongodb://localhost:27017/audittrace";
    await mongoose.connect(uri);
    console.log("Connected to MongoDB for seeding...");

    // Clear existing records
    await Invoice.deleteMany({});
    console.log("Cleared existing invoice documents.");

    // Insert 18 realistic sample invoices
    const inserted = await Invoice.insertMany(sampleInvoices);
    console.log(`Successfully seeded ${inserted.length} realistic historical invoices!`);
    console.log(`- Approved Invoices: ${inserted.filter((i) => !i.isFlagged).length}`);
    console.log(`- Flagged Discrepancies: ${inserted.filter((i) => i.isFlagged).length} (2 Duplicates, 1 Price Outlier)`);
